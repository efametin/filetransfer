...
async def komek(update: Update, context: CallbackContext):
    ...
    help_text = (
        "📌 Kömək Məlumatı:\n\n"
        "✅ Əgər oyunla bağlı sualınız varsa, whatsappdan yaza bilərsiniz..\n"
        "---\n"
        "⚠️ Yox əgər bu botda problem görmüsüzsə, söymüyün 😄 Yazın düzəldəcəm =)) 😉"
    )
...
async def sesver(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text("🗳 Oyunun ən yaxşı oyunçusunu (man of the match) seçmək üçün səs verin!", reply_markup=reply_markup)
...
async def announce_winner(chat_id, application):
    ...
    await application.bot.send_message(chat_id, f"🏆 Son oyunun ən yaxşı oyunçusu (man of the match) **{winner}** oldu! Təbriklər 🔥")
...
async def oyunagelirem(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"✅ {username} oyuna gəlməyini təsdiqlədi!")  # Təsdiq mesajı
...
async def mengelmirem(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"❌ {username} oyuna gəlməkdən imtina etdi!")
    await list_participants(update, context)
